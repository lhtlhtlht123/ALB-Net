...
class Trainer:
    def __init__(self, model, CFG=None, loss=None, train_loader=None, val_loader=None, save_tag=""):
        self.CFG = CFG
        ...
        self.checkpoint_dir = os.path.join(
            CFG.save_dir, self.CFG['model']['type'], save_tag + '_' + start_time)
    ...
    #         # 每隔10次迭代保存一个检查点
    #         if (i + 1) % 10 == 0:
    #             self._save_checkpoint(epoch, i + 1)
    #
    #     self.lr_scheduler.step()
    ...
    #     else:
    #         filename = os.path.join(self.checkpoint_dir,
    #                                 f'checkpoint-epoch{epoch}-iter{iteration}.pth')
    #     logger.info(f'Saving a checkpoint: {filename} ...')
    #     torch.save(state, filename)
    #     return filename
